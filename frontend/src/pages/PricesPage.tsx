          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/70 border-b border-gray-100">
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">TLD</th>
                <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Register</th>
                <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Renew</th>
                <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Transfer</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {visibleEntries.map(([tld, info]: [string, any]) => (
                <tr key={tld} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-4 py-3.5 text-sm font-semibold text-gray-900 whitespace-nowrap">.{tld}</td>
                  <td className="px-4 py-3.5 text-sm text-gray-700 text-right whitespace-nowrap">{fmtPrice(info.price_new || info.price_register, info.currency)}</td>
                  <td className="px-4 py-3.5 text-sm text-gray-700 text-right whitespace-nowrap">{fmtPrice(info.price_renew, info.currency)}</td>
                  <td className="px-4 py-3.5 text-sm text-gray-700 text-right whitespace-nowrap">{fmtPrice(info.price_transfer, info.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
