  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Link
              to={`/domains/${id}`}
              className="inline-flex items-center text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors bg-white px-2.5 py-1 rounded-md border border-gray-200 shadow-2xs"
            >
              <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Back to Domain
            </Link>
          </div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2 pt-1">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="outline" onClick={fetchAllRecords} disabled={loading} className="shadow-2xs">
            <RefreshCw className={`w-3.5 h-3.5 mr-1.5 inline ${loading ? "animate-spin" : ""}`} />
            Refresh
          </Button>
          <Button
            onClick={() => { setForm({ type: "a", hostname: "", value: "", ttl: 3600 }); setAddOpen(true); }}
            className="shadow-sm bg-black hover:bg-gray-800 text-white"
          >
      ...
      <Card className="p-0 overflow-hidden border border-gray-200/80 shadow-xs rounded-2xl">
        ...
        <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 bg-gray-50/70">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-white rounded-lg border border-gray-200 shadow-2xs">
              <Filter className="w-3.5 h-3.5 text-gray-500" />
            </div>
            <span className="text-xs font-semibold text-gray-700">Filter Record Type:</span>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs bg-white font-medium text-gray-800 shadow-2xs focus:outline-none focus:ring-2 focus:ring-black/10 focus:border-black"
            >
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200/60">
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-xs text-gray-400 font-medium">Fetching DNS records from Resellercamp...</span>
          </div>
          ...
          <div className="text-center py-20 px-4 space-y-3">
            ...
            >
              <Plus className="w-3.5 h-3.5 mr-1 inline" /> Add {filterType === "all" ? "DNS" : filterType.toUpperCase()} Record
            </Button>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                    ...
                    <th className="px-5 py-3.5 w-28">TTL</th>
                  ...
                  {filteredRecords.map((r, i) => (
                    <tr key={i} className="hover:bg-gray-50/60 transition-colors group">
                      ...
                      <td className="px-5 py-3.5 align-middle">
                        <span className={`inline-flex items-center justify-center px-2.5 py-1 font-mono font-bold text-[11px] uppercase rounded-md border shadow-2xs ${getBadgeStyle(r.type)}`}>
                          {r.type}
                        </span>
                      </td>

                      {/* Hostname */}
                      <td className="px-5 py-3.5 align-middle font-mono text-xs font-semibold text-gray-900">
                        {r.hostname}
                      </td>

                      {/* Value with copy button */}
                      <td className="px-5 py-3.5 align-middle font-mono text-xs text-gray-700 break-all max-w-md">
                      ...
                      {/* TTL */}
                      <td className="px-5 py-3.5 align-middle text-xs text-gray-500 font-mono">
                        <span className="inline-flex items-center px-2 py-0.5 bg-gray-100/70 rounded text-[11px] text-gray-600 font-medium">
                          {r.ttl}s
                        </span>
                      </td>
