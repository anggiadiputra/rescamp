...
import { sweepStuckPaidOrders, sweepUnresolvedRejectedPayments } from "./lib/payment-observability";
...
function startAutoExpireSweeper() {
  ...
  sweepUnresolvedRejectedPayments().catch((e) => console.warn("[sweeper] initial rejected-payment sweep failed:", e));
  sweepTimer = setInterval(() => {
    ...
    sweepUnresolvedRejectedPayments().catch((e) => console.warn("[sweeper] rejected-payment sweep failed:", e));
  }, 15 * 60 * 1000);
}
